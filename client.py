...
class AzureSearch(object):
        ...
        def __init__(self, serviceName, version, apiKey, timeout=60, max_retries=0):
        ...
        ## Manahe url
        def manageurl(self, subscriptionId, resourceGroupName):
            return "https://management.azure.com/subscriptions/" + subscriptionId + "/resourceGroups/" + resourceGroupName + "/providers/Microsoft.Search/searchServices/" + self.serviceName + "?" + self.version

        def manageadminkey(self, subscriptionId, resourceGroupName):
            return "https://management.azure.com/subscriptions/" + subscriptionId + "/resourceGroups/" + resourceGroupName + "/providers/Microsoft.Search/searchServices/" + self.serviceName + "/listAdminKeys?" + self.version
        ...
        def dateToday(self):
            d = datetime.now()
            now = str(d.year)+"-"+str(d.month)+"-"+str(d.day)+"T"+str(d.hour)+":"+str(d.minute)+":"+str(d.second)
            return now
        ...
        def deleteindex(self, indexname):
        	...
        	See List Indexes (Azure Search API) for details.
        	"""
    ...
    ##Search Documents
        def simplesearch(self, string):
            query = "search=" + string
            url =  self.searchbyurl("reezocar", query)
            self._send_request(url, "GET") 
	...
	## Index Admin API
        def count(self):
                ...
                """
                url = self.geturl() + '/docs/$count?api-version=2014-07-31-Preview'
                self._send_request(url, "GET")

        def createSearchService(self, subscriptionId, resourceGroupName, body):
                """
                ...
                """
                value = self.encode_json(body)
                url = self.manageurl(subscriptionId, resourceGroupName)
                print url
                self._send_request(url, "PUT", value)

        def listAdminKeys(self, subscriptionId, resourceGroupName):
                """
                ...
                """
                url = self.manageadminkey(subscriptionId, resourceGroupName)
                print url
                self._send_request(url, "POST")

        def getSearchService(self, subscriptionId, resourceGroupName):
            """
        ...
        def createQueryKey(self, subscriptionId, resourceGroupName, name):
            """
            The Create Query Key operation generates a new query key for the Search service. You can create 
        ...
        def deleteQueryKey(self, subscriptionId, resourceGroupName, key):
        	...
        	arg subscriptionId: The subscriptionID for the Azure user. You can obtain this value from the Azure Resource Manager API or the portal.
        	arg resourceGroupName:  The name of the resource group within the user’s subscription. You can obtain this value from the Azure Resource Manager API or the portal.
